...
impl Foo {
    ...
    //               ↑ 规则3：有 &self 时，返回引用自动取 self 的生命周期 'a。
    //                 若改返回 _x（&'b str），生命周期不匹配，编译器报 "lifetime may not live long enough"。
    {
        &self.inner
    }
}
...
3. 当两个引用共享同一个生命周期参数时，编译器取其**较短者**作为有效范围。因此 `longest(&s1, &s2)` 中 `result` 的使用范围不超过 s1 和 s2 中较短者的存活期。
