...
| 第二章 | 可变栈 | `Option::take()` | ⭐⭐ |
| 第三章 | 持久化栈 | `Rc` | ⭐⭐ |
| 第四章 | 安全双向 Deque | `Rc<RefCell<Node>>` | ⭐⭐⭐ |
| 第五章 | 不安全单链表队列 | `*mut Node<T>` | ⭐⭐⭐⭐ |
| **第六章** | **生产级双向 Deque** | `NonNull<T>` + `PhantomData` | ⭐⭐⭐⭐⭐ |
...
> **译注**：如果你还没读过第四章和第五章，强烈建议先回顾。第四章展示了 `Rc<RefCell<Node>>` 方案的局限性；第五章引入了裸指针和 `Box::into_raw`/`from_raw` 的所有权转移技巧。本章将把这些概念推向极致。
...
## 与第四章 Safe Deque 的终极对比

> **译注**：本章多次提到"第四章"的 `Rc<RefCell<Node>>` 方案，但请注意：本地仓库的 [fourth.md](fourth.md) 是 unsafe 队列实现（对应 too-many-lists 第五章），而非 `Rc<RefCell>` 双向 Deque。too-many-lists 原书中真正的 `Rc<RefCell>` 双向 Deque 章节在[这里](https://rust-unofficial.github.io/too-many-lists/fourth.html)，建议对照阅读。

| 特性 | 第四章（`Rc<RefCell>`） | 第六章（Production Unsafe） |
|------|---------------------|--------------------------|
| 运行时开销 | 引用计数 + 借用检查 | 无 |
...
六章下来，我们从最天真的 `enum List` 走到了生产级的 `LinkedList<T>`。这条路径上的每一个坑，都是 Rust 所有权系统的教学案例：
...
| 第四章 | 运行时借用检查（`RefCell`）能绕过编译器，但代价是性能和 API 受限 |
| 第五章 | 裸指针是处理自引用和 O(1) 尾操作的最终手段 |
| 第六章 | `NonNull` + `PhantomData` + 手工 `Send`/`Sync` = 生产级 unsafe 抽象 |

Rust 的 unsafe 边界不是"安全 vs 不安全"的二元对立，而是一个**频谱**：

- **完全 safe**：编译器验证一切（第二章的栈）
- **safe 但 bad**：编译器通过，但运行时开销高、API 受限（第四章的 `Rc<RefCell>` deque）
- **unsafe 但 sound**：程序员验证不变量，对外暴露 safe API（第五章的队列、第六章的 deque）
