...
| 函数返回两个来源的引用且需表达 outlives 关系 | 需要独立生命周期参数并声明 `'b: 'a` 等约束 | 无需考虑 |
...
方案 2：如果两个输入有**不同**生命周期、且返回值只绑定其中**较长者** `y`，需要声明 `y` 的生命周期 outlives 返回值。注意：这**不适用于**上面的 `main` 示例，因为那里的 `s2` 比 `s1` 短；下面是它真正能编译的调用场景。
...
fn main() {
    let s2 = String::from("world");          // y：较长
    {
        let s1 = String::from("hello");      // x：较短
        let result = longer_v2(&s1, &s2);
        println!("{result}");                // result 绑定 s2 的生命周期
    }  // s1 在这里 drop，不影响 result
}
```

> 总结：当函数可能返回任一输入时，返回值生命周期天然受限于**最短的那个输入**。生命周期标注不能绕过这条规则，只能显式约束来源之间的关系。如果实际业务需要返回一个比某个输入活得更久的引用，应选择方案 1（缩小使用范围）或返回 `String`（交出所有权）。