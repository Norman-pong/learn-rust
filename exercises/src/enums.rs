...
fn enums1() {
    ...
    let text = match msg {
        Message::Quit => todo!("返回空字符串"),
        Message::ChangeColor(_, _, _) => todo!("返回描述"),
        Message::Write(s) => todo!("返回 s, 原断言: assert_eq!(text, \"hello\")"),
    };
}
...
fn enums2() {
    ...
    let result = if let Some(v) = maybe_value {
        todo!("返回 v 的两倍, 原断言: assert_eq!(result, 84)")
    } else {
        todo!("返回 0")
    };
}
    ...
    impl Operation {
        fn compute(&self) -> i32 {
            match self {
                ...
                Operation::Multiply(a, b) => todo!("返回 a * b, 原断言: assert_eq!(op.compute(), 42)"),
            }
        }
    }
